...
template<bool SKIP> static void GPU_RenderLine_DispCapture(u16 l)
{
   ...
   const display_control_t display_control = gpu->get_display_control();
   const capture_control_t control = gpu->get_capture_control();
   const u32 read_block = display_control.VRAM_Block;
   ...
   static const struct size_tag { u32 x; u32 y; } capture_sizes[4] = { { 128, 128 }, { 256, 64 }, { 256, 128 }, { 256, 192 } };
   const size_tag& capture_size = capture_sizes[control.size];
   ...
   if (control.enabled)
   {
      ...
      u32 cap_src_adr = control.read_offset  * 0x8000 + (l * 512);
      u32 cap_dst_adr = control.write_offset * 0x8000 + (l * (capture_size.x * 2));
      ...
      cap_src_adr += read_block  * 0x20000;
      cap_dst_adr += control.write_block * 0x20000;
      ...
      const bool write_blocked = vramConfiguration.banks[control.write_block].purpose != VramConfiguration::LCDC;

      //we must return zero from reads from memory not mapped to lcdc
      if(vramConfiguration.banks[read_block].purpose != VramConfiguration::LCDC)
         cap_src = MMU.blank_memory;

      if (!SKIP && !write_blocked && l < capture_size.y)
      {
         if (control.capture_source == 0)
         {
            if (!control.source_a)
            {
            ...
            {
               u16* src;
               gfx3d_GetLineData15bpp(l, &src);
               CAPCOPY((u8*)src, cap_dst, false);
            }
         }
         else if (control.capture_source == 1)
         {
            if (!control.source_b)
            {
               CAPCOPY(cap_src, cap_dst, true);
            }
            else //capture dispfifo (not yet tested)
            {
               for(int i=0; i < 128; i++)
         ...
         else
         {
            ...
            if (control.source_b == 0)         // VRAM screen
               srcB = (u16 *)cap_src;
            else
            {
            ...
            for(u32 i = 0; i < capture_size.x; i++) 
            {
               u16 a,r,g,b;
               ...
               if(a_alpha)
               {
                  const u32 eva = control.get_eva();
                  a = 0x8000;
                  r = ((srcA[i] & 0x1F) * eva);
                  g = (((srcA[i] >>  5) & 0x1F) * eva);
                  b = (((srcA[i] >>  10) & 0x1F) * eva);
               } 
               else
                  a = r = g = b = 0;
               ...
               if(b_alpha)
               {
                  const u32 evb = control.get_evb();

                  a = 0x8000;
                  r += ((srcB[i] & 0x1F) * evb);
                  g += (((srcB[i] >>  5) & 0x1F) * evb);
                  b += (((srcB[i] >> 10) & 0x1F) * evb);
               }
      ...
      if (l >= 191)
      {
         gpu->dispx_st->capture_control.enabled = false;
      }
   }
}
...
void GPU_RenderLine(NDS_Screen * screen, u16 l, bool skip)
{
   ...
   if(master_bright.max && (master_bright.mode == 1 || master_bright.mode == 2))
   {
      ...
      if(!(gpu->core == GPU_MAIN && (gpu->get_capture_control().enabled || l == 0 || l == 191)))
      {
         gpu->currLine = l;
